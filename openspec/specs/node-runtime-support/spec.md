...
- **WHEN** a developer runs `bun run start` (or `bun cli.ts`)
...
- **THEN** it produces a self-contained `dist/cli.js` (with `server.ts` bundled in) that runs under Node 18+
...
- **THEN** `bin` points to the compiled entry (`dist/cli.js`), `engines.node` is `>=18`, and there is no self-referential dependency on `@sflueckiger/specboard`
